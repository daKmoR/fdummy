...
/*		$gridItems = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows(
				'uid, title, description, icon',
				'tx_gridelements_backend_layout',
				'NOT hidden AND NOT deleted AND uid NOT IN (' . $excludeList . ')',
				'',
				'title ASC'
		);
*/
		...
		if(count($gridItems)) {
			...
			foreach($gridItems as $key => $item) {
				// Icon path parsed already in tx_gridelements_layoutsetup
